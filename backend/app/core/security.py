...
def create_refresh_token(user: User, db: Session):
    ...
    try:
        private_key = _load_private_key()
        encoded_jwt = jwt.encode(to_encode, private_key, algorithm=settings.ALGORITHM)
        db.add(RefreshToken(hashed_jti=hash_jti(jti), user_id=user.id, expires_at=expire))
        db.commit()
        return encoded_jwt
    except Exception as e:
...
def is_access_token_valid(token: str) -> bool:
    ...
    if not payload:
        return False
    return payload["type"] == "access" and payload["sub"] and payload["exp"] > datetime.now(timezone.utc)
...
def is_refresh_token_valid(token: str, db: Session) -> bool:
    ...
    matching_record = db.query(RefreshToken).filter(RefreshToken.hashed_jti == hash_jti(payload["jti"])).first()
    return matching_record and matching_record.expires_at > datetime.now(timezone.utc) and not matching_record.is_revoked
